...
const backupPath = path.resolve(__dirname, 'blog.db.backup');
if (fs.existsSync(dbPath)) {
    fs.copyFileSync(dbPath, backupPath);
    console.log('Database backup created at blog.db.backup');
}
...
// Populate Loop
db.serialize(() => {
    // Drop existing table to reset content
    ...
    posts.forEach(post => {
        stmt.run(post.title, post.content, post.author, today, post.tags, post.image);
    });

    stmt.finalize();
    console.log(`Successfully populated ${posts.length} blog posts with verified Unsplash images.`);
});
