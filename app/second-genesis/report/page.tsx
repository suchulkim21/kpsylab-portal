...
import { Lock, RefreshCw, ShieldAlert, Activity, Brain, Fingerprint, ArrowRight, CheckCircle } from "lucide-react";
import {
    Radar, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, ResponsiveContainer,
    Legend, Tooltip
} from 'recharts';
import { generateFinalReportContent } from "@second-genesis/lib/content/finalReport";
import { getKoreanTypeName, determineTypeCode } from "@second-genesis/lib/content/module2";
import { FeedbackVariant } from "@second-genesis/lib/content/finalAnalysisEngine";
import { CompositeAnalysisEngine } from "@second-genesis/lib/CompositeAnalysisEngine";
...
export default function FinalReportPage() {
    ...
    const runAnalysisWithData = (data: ScoreBundle | null, answer: string, gapOverride: string) => {
        try {
            ...
            // Type Calculation
            const typeCode = determineTypeCode({ p, a, sd });
            const m2TypeName = getKoreanTypeName(typeCode);
        ...
        return (
            <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center text-white p-6 relative overflow-hidden">
                ...
                <div className="z-10 bg-black/50 border border-red-900/50 p-12 rounded-lg backdrop-blur-md flex flex-col items-center max-w-lg w-full">
                    ...
                    <Link href="/second-genesis" className="px-8 py-3 bg-red-900/20 border border-red-500/30 text-red-400 hover:bg-red-500 hover:text-white rounded transition-all font-mono text-xs uppercase tracking-widest">
                        대시보드로 복귀
                    </Link>
                </div>
            </div>
